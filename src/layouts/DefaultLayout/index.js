...
function DefaultLayout({ children, keySearch, setKeySearch }) {
  ...
  const getBySenderIdAndReceiveId = async (userId1, userId2) => { // get
    axios.defaults.headers.common["Authorization"] = `Bearer ${JSON.parse(
      localStorage.getItem("userToken")
    )}`;
    try {
      const response = await axios.get(
        `http://localhost:8080/friendrequest/findBySenderIdAndReceiverId/?senderId=${userId1}&receiverId=${userId2}`
      );
  ...
  const deleteFriendRequest = async (requestId) => { // delete
    axios.defaults.headers.common["Authorization"] = `Bearer ${JSON.parse(
      localStorage.getItem("userToken")
    )}`;
    try {
  ...
  // user following
  const saveUserFollowing = async (userFollowingId , userFollowerId) => {     // create
    const request = {
        followingId: null,
        userId: userFollowingId,
        followingUserId: userFollowerId,
        followDate: null
    };
  ...
  const saveFriendShip = async (userId1 , userId2) => { // create
    const request = {
      friendshipId: null,
      userId1: userId1,
      userId2: userId2,
  ...
  const handleAcceptButtonFR = async (item) => {
    ...
    await saveUserFollowing(item.receiverId , item.senderId);

    const friendRequest = await getBySenderIdAndReceiveId(item.senderId, item.receiverId);
    await deleteFriendRequest(friendRequest.requestId);
};
    ...
    <div className={style.container}>
      ...
      <div ref={fastNotificationRef} className={style.notification}>
        {notification.notificationId !== null && (
          ...
          <div className={style.button}>
            <button onClick={() => handleAcceptButtonFR(notification)}  className={style.acceptButton}>
              Accept
            </button>
            <button onClick={() => handleDeleteButtonFR(notification)} className={style.deleteButton}>
              Delete
            </button>
          </div>
        </div>
