...
export function People() {
    ...
    const findUserByKey = async () => {
        ...
        try {
            const response = await axios.get("http://localhost:8080/user/searchUserByKey/?keySearch=" + keySearch);
                ...
                <div className={style.friendHeader}>
                    ...
                    <div className={style.col2}>
                        <div className={style.search}>
                            <input value={keySearch} onChange={(e) => setKeySearch(e.target.value)} placeholder='Find Friends...'></input>
                        ...
                        return (
                            <div key={index} className={style.item}>
                                ...
                                <div className={style.viewProfile}>
                                    <button>  <Link className='link' to={`${routes.userWall}?id=${item.id}`}>View Profile</Link></button>
                                </div>
                            </div>
                        )
                    })}
